...
INNER JOIN escuela as ESC on ESC.id_esc = est.id_esc
 ...
 --Que estudiantes(nombres, apellidos,direccion)
 --forman parte de la escuela 'San Felipe Asturias II'
 
 --1ª paso: Identificar que campos tengo que mostrar
 --nombre,apellidos,direccion

 --2º paso: En que tablas estan estos campos
 --ESTUDIANTES
 --ESCUELA

 --3º PASO:	 Estas tablas estanrelacionadas entresi?
 --INNER JOIN
...
INNER JOIN kardex as kar on kar.id_est_cedula = est.id_est_cedula
 where kar.nota_num >= 51 and est.edad>=25
   ...
   select mat.semestre,mat.ano_gestion
from estudiantes as  est
INNER JOIN kardex as kar on kar.id_est_cedula = est.id_est_cedula
inner join periodo as mat on mat.id_per = kar.id_per
